...
| 描述                | 结果类型 | 描述作用                | 举例                   |
|-------------------|:----:|---------------------|----------------------|
| role.info.name    | 字符串  | 角色名称                | role.info.name       |
| role.info.setName |  无   | 设定角色名称              | role.info.setName 小明 |
...
| role.bag.remove |  无   | 为角色移除道具或减少数量。移除数量默认为1          | role.bag.remove 金币 1 |
...
| nextEvent       |  事件  | 下一个可触发的事件，并会触发一次此事件（每回合只会触发同一个事件一次）       | nextEvent                                |
